...
public:
	...
	Student(int rollno, char name[20], char city[20], char year[20],
			char div[20]) {
	...
	void displayRecord() {
		...
		cout << endl << setw(5) << rollno << setw(10) << name << setw(9) << year
				<< setw(13) << div << setw(10) << city;
...
public:
	...
	void insertRecord(int rollno, char name[20], char year[20], char div[20],
			char city[20]) {
	...
	void displayRecord(int rollNo) {
		...
		while (file.read((char*) &s1, sizeof(Student))) {
			if (s1.getRollNo() == rollNo) {
				cout << "\n---------------------------------------------------";
				cout << endl << setw(5) << "ROLL" << setw(10) << "NAME"
						<< setw(10) << "YEAR" << setw(10) << "DIV" << setw(10)
						<< "CITY";
				cout << "\n---------------------------------------------------";
				s1.displayRecord();
				flag = true;
...
int main() {
	fstream newFile("student.txt", ios::app | ios::binary);
	...
	while (choice != 6) {
		...
		case 5:
			...
			if (newFile.tellg() == 0) {
				...
				cout << "\nEnter Roll_No Which Record You Want To Delete: ";
				cin >> rollNo;
